...
import {
  showUploadModal,
  closeUploadModal,
} from "../../connected-components/UploadModal/actions";
...
setimageDisabled(true)
  ...
  return (
    <Container>
      {!files.length && <UploadContainer animation={
        "fadeInUp"
      }>
        <Circle style={circleStyle} onPress={() => type === 'file' ? showUploadSheet() : cameraLaunch()}  disabled={imageDisabled}>
          <Icon
        ...
        {showUploadBuildingPhotoButton && type === 'photo' && <Button
          label={tr('upload.uploadBuildingPhotos')}
          type={"solid"}
          labelStyle={{
      ...
      <RowView>
        {files?.map((file, idx) => {
          ...
          return (
            file.type === 'image' ? (<SelectedImageContainer key={idx}>
              {showCloseButton &&
              <CloseContainer onPress={() => removeItem(file)}>
                <Icon name={'close'} type={'AntDesign'} size={getByScreenSize(theme.text.s9, theme.text.s10)} color={'white'} />
              </CloseContainer>
              }
              <Image source={{ uri: file.uri }} />
            </SelectedImageContainer>) : ( 
            <SelectedItemPdfContainer  key={idx} style={{backgroundColor: icon.background}}>
            <CloseContainer onPress={() => removeItem(file)}>
              <Icon  name={'close'}type={'AntDesign'} size={getByScreenSize(theme.text.s9, theme.text.s10)} color={'white'} />
            </CloseContainer>
            <Icon name={icon.name} type={icon.type} size={theme.text.s3} color={icon.color}
            />
          </SelectedItemPdfContainer>
            )
          )
        })}
  {files.length==0?false:addMultiplePhotos  && files.length < 5 && <SelectedItemContainer style={{ backgroundColor: theme.files.addBackground, marginRight: '2%' }} onPress={() => cameraLaunch()} disabled={imageDisabled}>
          <Icon name={'add-circle-outline'} type={'Ionicons'} size={theme.text.s3} color={theme.primary} />
        </SelectedItemContainer>}
      </RowView>
      {showUploadButton && files.length > 0 && <Button 
        label={tr('upload.uploadBuildingFiles')}
        type={"solid"}
        labelStyle={{
          fontSize: getByScreenSize(theme.text.s7, theme.text.s10),
        }}
...
export default connect(null, { showUploadModal, closeUploadModal })(CameraButton);